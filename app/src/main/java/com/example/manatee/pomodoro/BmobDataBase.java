...
public class BmobDataBase {
    ...
    private List<BmobObject> remoteInsertList = new ArrayList<BmobObject>();
    private List<BmobObject> remoteUpdateList = new ArrayList<BmobObject>();
    private List<BmobObject> remoteDeleteList = new ArrayList<BmobObject>();
    private List<ScheduleUnit> remoteList = new ArrayList<ScheduleUnit>();
    private List<ScheduleUnit> localList = new ArrayList<ScheduleUnit>();
    private boolean InsertFinish;
    private boolean UpdateFinish;
    private boolean DeleteFinish;
    private boolean alreadyFailed;                              //为了只让同步失败显示一次
    ...
    private void UpLoadSync() {
        ...
        for (Iterator<ScheduleUnit> it = localList.iterator(); it.hasNext(); ) {
            ...
            if (unitRemote == null) {                                         //云端没有，则需要insert
                ...
                boolean flag = localPO.isSame(remotePO);
                if (flag == false) {                                            //云端有，但数据和本地不同，需要update
            ...
            if (remoteInsertList.size() == 50) {                              //满50个就要上传一次
                insertRemote();
            }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case UpLoadRemoteError:
                    if (!alreadyFailed){                                                    //增删改都要报这个错误，所以需要置位
                        Toast.makeText(mContext, "上传云端出错", Toast.LENGTH_SHORT).show();//alreadyFailed让Toast只显示一次
                        alreadyFailed = true;
                    }
                    break;
                case UpLoadRemoteFinish:
                    if ( InsertFinish && UpdateFinish && DeleteFinish )
                        Toast.makeText(mContext, "已同步到云端", Toast.LENGTH_SHORT).show();
                    break;
                default:
                    break;
            }
